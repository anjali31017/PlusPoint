...
async def create_user(user_data: UserCreateSchema, background_tasks: BackgroundTasks):
    try:
        ...
        if user is None:
            ...
            detail="User creation failed or user already exists"
        )
    ...
    try:
        ...
        if not user.verify_otp(otp_data.otp):
            ...
            if attempts >= 5:
                ...
                raise HTTPException( status_code=status.HTTP_400_BAD_REQUEST,detail="Too many invalid OTP attempts. User blocked for 6 hours." )
        
            # await user.set({UserModel.is_verified: True, UserModel.is_active: True})
            raise HTTPException( status_code=400, detail=f"Invalid OTP. {5 - attempts} attempts remaining" )
