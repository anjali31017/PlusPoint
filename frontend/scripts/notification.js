...
//     switch (n.type) {
//         case "ARTICLE":
//             return `📰 New article published: <b>${m.article_title || "Article"}</b> by <i>${m.article_firm || "Article"}</i>`;
...
//         case "ADMIN":
//             return `
//                 🛡️ <b>${m.status || "Admin update"}</b>
//                 ${m.detail ? `<br/><span class="text-sm text-gray-500">${m.detail}</span>` : ""}
//             `;
...
//         // case "ADMIN":
//         //     if (m.article_id) window.location.href = `article.html?article_id=${m.article_id}`;
//         //     else if (m.firm_id) window.location.href = `firm.html?firm_id=${m.firm_id}`;
//         //     else Swal.fire({ icon: "info", title: m.status, text: m.detail || "" });
//         //     break;
//     }
// }
